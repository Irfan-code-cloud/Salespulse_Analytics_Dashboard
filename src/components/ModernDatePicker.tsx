...
export const ModernDatePicker: React.FC<ModernDatePickerProps> = ({ label, value, onChange }) => {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <div 
        ...
        className={cn(
          "flex items-center gap-[0.75rem] bg-white border border-transparent rounded-[0.5rem] shadow-sm px-[1rem] py-[0.75rem] cursor-pointer transition-all hover:border-[#141414]/10",
      ...
      >
        <div className="flex items-center gap-[0.5rem] flex-shrink-0">
          ...
          <span className="text-[0.7rem] font-bold uppercase tracking-widest text-[#141414]/40">{label}</span>
        </div>
        <div className="text-[0.875rem] font-medium text-[#141414]">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-50 top-full left-0 mt-[0.5rem] bg-white border border-[#141414]/10 rounded-[1rem] shadow-xl p-[1rem]"
          >
            <DayPicker
              mode="single"
              ...
              styles={{
                caption: { color: '#141414', fontWeight: 'bold' },
